...
import pages.identification.CustomsofficePage
import play.api.i18n.{I18nSupport, MessagesApi}
import play.api.mvc.{Action, AnyContent, MessagesControllerComponents}
import repositories.SessionRepository
import services.CustomsOfficesService
import uk.gov.hmrc.play.bootstrap.frontend.controller.FrontendBaseController
import views.html.identification.CustomsofficeView
  ...
  def onPageLoad(mrn: MovementReferenceNumber, mode: Mode): Action[AnyContent] = actions.requireData(mrn).async {
    implicit request =>
      service.getCustomsOfficesOfArrival.map {
        customsOfficeList =>
          val form = formProvider("identification.customsoffice", customsOfficeList)
