...
class CheckIncidentAnswersController @Inject() (
  override val messagesApi: MessagesApi,
  navigatorProvider: IncidentsNavigatorProvider,
  actions: Actions,
  ...
  def onPageLoad(mrn: MovementReferenceNumber, mode: Mode, index: Index): Action[AnyContent] = actions.requireData(mrn) {
    implicit request =>
      val sections = viewModelProvider(request.userAnswers, index, mode).sections
      Ok(view(mrn, mode, index, sections))
  }

  def onSubmit(mrn: MovementReferenceNumber, mode: Mode, index: Index): Action[AnyContent] = actions.requireData(mrn) {
    implicit request =>
      Redirect(navigatorProvider(mode).nextPage(request.userAnswers, Some(IncidentSection(index))))
  }
