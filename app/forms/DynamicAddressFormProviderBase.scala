...
abstract class DynamicAddressFormProviderBase extends Mappings {

  def postalCodeLength: Int
      ...
      mapping(
        ...
        PostalCode.field -> {
          val constraint = StopOnFirstFail[String](
            maxLength(PostalCode.length, s"$prefix.error.length", Seq(PostalCode.arg) ++ args ++ Seq(postalCodeLength)),
