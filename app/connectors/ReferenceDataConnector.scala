...
import models.reference.{Country, CountryCode, CustomsOffice, Nationality, UnLocode}
...
class ReferenceDataConnector @Inject() (config: FrontendAppConfig, http: HttpClient, metricsService: MetricsService) {
  ...
  def getCustomsOfficesForCountry(countryCode: CountryCode)(implicit ec: ExecutionContext, hc: HeaderCarrier): Future[Seq[CustomsOffice]] = {
    ...
    metricsService.timeAsyncCall(Monitors.getCustomsOfficesForCountryMonitor) {
  ...
  def getNationalities()(implicit ec: ExecutionContext, hc: HeaderCarrier): Future[Seq[Nationality]] = {
    ...
    http.GET[Seq[Nationality]](serviceUrl)
  }
