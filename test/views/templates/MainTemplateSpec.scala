...
class MainTemplateSpec extends SpecBase with ViewSpecAssertions with AppWithDefaultMockFixtures with Generators {
  ...
  "when not in trader test" - {
    ...
    "must point feedback at feedback form" in {
      ...
      forAll(Gen.alphaNumStr, Gen.alphaNumStr, Gen.option(arbitrary[MovementReferenceNumber])) {
        (content, title, mrn) =>
          val view = app.injector
            .instanceOf[MainTemplate]
            .apply(title, timeoutEnabled = true, showBackLink = true, mrn) {
              Html.apply(content)
            }
    ...
    "when session has an auth token" - {
      ...
      "must render timeout dialog and sign out link" - {
        "when mrn defined" in {
          forAll(Gen.alphaNumStr, Gen.alphaNumStr, arbitrary[MovementReferenceNumber]) {
            (content, title, mrn) =>
              val view = app.injector
                .instanceOf[MainTemplate]
                .apply(title, timeoutEnabled = true, showBackLink = true, Some(mrn)) {
                  Html.apply(content)
                }
              ...
              assertElementExists(metas, _.attr("data-keep-alive-url") == s"/manage-transit-movements/arrivals/$mrn/keep-alive")

              val link = getElementByClass(doc, "hmrc-sign-out-nav__link")
              assertElementContainsText(link, "Sign out")
              assertElementContainsHref(
                link,
                controllers.routes.DeleteLockController.delete(mrn, None).url
              )
          }
        }
        ...
        "when mrn undefined" in {
          forAll(Gen.alphaNumStr, Gen.alphaNumStr) {
            (content, title) =>
              ...
              val metas = getElementsByTag(doc, "meta")
              assertElementExists(metas, _.attr("name") == "hmrc-timeout-dialog")
              assertElementExists(metas, _.attr("data-keep-alive-url") == "/manage-transit-movements/arrivals/keep-alive")
