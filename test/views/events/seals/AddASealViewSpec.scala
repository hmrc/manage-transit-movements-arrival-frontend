...
class AddASealViewSpec extends SingleViewSpec("events/seals/addSeal.njk") {
  ...
  "must display the add another seal Yes/No radio when below maximum item limit" in {
    val baseJson =
      Json.obj(
        "allowMoreSeals" -> true
      )
  ...
  "must not display the add another seal Yes/No radio when reached maximum item limit" in {
    val baseJson =
      Json.obj(
        "allowMoreSeals" -> false
      )
