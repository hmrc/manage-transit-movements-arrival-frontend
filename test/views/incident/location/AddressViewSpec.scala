...
class AddressViewSpec extends DynamicAddressViewBehaviours with Generators {
  ...
  override def applyView(form: Form[DynamicAddress]): HtmlFormat.Appendable =
    injector.instanceOf[AddressView].apply(form, mrn, NormalMode, index, isPostalCodeRequired)(fakeRequest, messages)

  override val prefix: String = "incident.location.address"
