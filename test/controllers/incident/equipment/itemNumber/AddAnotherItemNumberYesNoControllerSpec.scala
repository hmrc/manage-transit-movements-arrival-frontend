...
class AddAnotherItemNumberYesNoControllerSpec extends SpecBase with AppWithDefaultMockFixtures with Generators {
  ...
  private def form(viewModel: AddAnotherItemNumberViewModel) = formProvider(viewModel.prefix, viewModel.allowMoreItems)

  private val mode                                = NormalMode
  private lazy val addAnotherItemNumberYesNoRoute = routes.AddAnotherItemNumberYesNoController.onPageLoad(mrn, mode, incidentIndex, equipmentIndex).url
  private val mockViewModelProvider               = mock[AddAnotherItemNumberViewModelProvider]

  override def guiceApplicationBuilder(): GuiceApplicationBuilder =
    super
      .guiceApplicationBuilder()
      .overrides(bind(classOf[AddAnotherItemNumberViewModelProvider]).toInstance(mockViewModelProvider))
  ...
  private val listItems         = Seq.fill(Gen.choose(1, frontendAppConfig.maxNumberOfItems - 1).sample.value)(listItem)
  private val maxedOutListItems = Seq.fill(frontendAppConfig.maxNumberOfItems)(listItem)
  ...
  "AddAnotherItemNumberYesNo Controller" - {
    ...
    "redirect to add item number yes/no page" - {
      "when 0 item numbers" in {
        ...
        val request = FakeRequest(GET, addAnotherItemNumberYesNoRoute)
    ...
    "must return OK and the correct view for a GET" - {
      "when max limit not reached" in {
        ...
        val view = injector.instanceOf[AddAnotherItemNumberYesNoView]
    ...
    "when max limit not reached" - {
      "when yes submitted" - {
        "must redirect to item number page at next index" in {
          ...
          val request = FakeRequest(POST, addAnotherItemNumberYesNoRoute)
            .withFormUrlEncodedBody(("value", "true"))
