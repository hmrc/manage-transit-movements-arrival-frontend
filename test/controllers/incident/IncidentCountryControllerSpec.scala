...
class IncidentCountryControllerSpec extends SpecBase with AppWithDefaultMockFixtures with Generators {
  ...
  private lazy val incidentCountryRoute              = routes.IncidentCountryController.onPageLoad(mrn, mode, index).url
  ...
  "IncidentCountry Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      contentAsString(result) mustEqual
        view(form, mrn, countryList.values, mode, index)(request, messages).toString
    }

    "must populate the view correctly on a GET when the question has previously been answered" in {

      when(mockCountriesService.getTransitCountries()(any())).thenReturn(Future.successful(countryList))
      val userAnswers = emptyUserAnswers.setValue(IncidentCountryPage(index), country1)
      ...
      contentAsString(result) mustEqual
        view(filledForm, mrn, countryList.values, mode, index)(request, messages).toString
    }
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      val request   = FakeRequest(POST, incidentCountryRoute).withFormUrlEncodedBody((field, "invalid value"))
      ...
      contentAsString(result) mustEqual
        view(boundForm, mrn, countryList.values, mode, index)(request, messages).toString
    }
