...
class IncidentFlagControllerSpec extends SpecBase with AppWithDefaultMockFixtures with MockitoSugar {
  ...
  "IncidentFlag Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      val request = FakeRequest(GET, incidentFlagRoute)
      val result  = route(app, request).value

      val view = injector.instanceOf[IncidentFlagView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.setValue(IncidentFlagPage, true)
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      val request =
        FakeRequest(POST, incidentFlagRoute)
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      val request   = FakeRequest(POST, incidentFlagRoute).withFormUrlEncodedBody(("value", ""))
    ...
    "must redirect to Session Expired for a POST if no existing data is found" in {
      ...
      val request = FakeRequest(POST, incidentFlagRoute)
        .withFormUrlEncodedBody(("value", "true"))
